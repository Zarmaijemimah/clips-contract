...
/// Parameters required to initialize the contract. (#452)
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InitParams {
    ...
    /// Default royalty in basis points (0–10_000)
    pub default_royalty_bps: u32,
    /// Optional backend Ed25519 signer public key. When provided, set during
    /// init so the contract can mint immediately without a separate set_signer call.
    pub signer: Option<BytesN<32>>,
}

/// Emitted after successful initialization. (#450)
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl ClipsNftContract {
    ...
    pub fn init(env: Env, params: InitParams) -> Result<(), Error> {
        if env.storage().instance().has(&DataKey::Admin) {
            return Err(Error::AlreadyInitialized);
        }
        // Auth check first so unauthorised callers get a clear rejection before
        // any validation or storage work is performed.
        params.admin.require_auth();
        Self::validate_init_params(&env, &params)?;
        ...
        env.storage().instance().set(&DataKey::PlatformRecipient, &params.admin);
        env.storage().instance().set(&DataKey::PlatformFeeBps, &params.platform_fee_bps);
        env.storage().instance().set(&DataKey::DefaultRoyaltyBps, &params.default_royalty_bps);
        env.storage().instance().set(&DataKey::DefaultRoyaltyAsset, &Option::<Address>::None);
        env.storage().instance().set(&DataKey::Name, &params.name);
        env.storage().instance().set(&DataKey::Symbol, &params.symbol);
        ...
        env.storage().instance().set(&DataKey::BackendAddress, &params.admin);
        env.storage().instance().set(&DataKey::ContractVersion, &VERSION);

        if let Some(pk) = params.signer {
            env.storage().instance().set(&DataKey::Signer, &pk);
        }

        env.events().publish(
            (symbol_short!("clips"), symbol_short!("init"),),
            InitializedEvent {
                owner: params.admin,
    ...
    /// Validate all initialization parameters. (#452)
    fn validate_init_params(env: &Env, params: &InitParams) -> Result<(), Error> {
        ...
        if params.platform_fee_bps > 10_000 {
            return Err(Error::InvalidConfiguration);
        }
        if params.default_royalty_bps > 10_000 {
            return Err(Error::InvalidConfiguration);
        }
        // Validate admin is a proper serialisable address; reject zero/invalid addresses.
        let xdr = params.admin.clone().to_xdr(env);
        if xdr.len() == 0 {
...
mod tests {
    ...
    fn do_init(client: &ClipsNftContractClient, env: &Env, admin: &Address) {
        client.init(&InitParams {
            admin: admin.clone(),
            name: String::from_str(env, "ClipCash Clips"),
            symbol: String::from_str(env, "CLIP"),
    ...
    fn test_calculate_royalty_amount_basic() {
        ...
        client.init(&InitParams { admin: admin.clone(), name: String::from_str(&env, "ClipCash Clips"), symbol: String::from_str(&env, "CLIP"), platform_fee_bps: 100, default_royalty_bps: 0, signer: None });
